// Team Members carousel navigation (scroll-snap based).
(function () {
	function initCarousel(root) {
		var track = root.querySelector('.bkbg-tm-track');
		if (!track) return;

		var settingsRoot = root.closest('.bkbg-tm-wrap') || root;
		var infiniteEnabled = settingsRoot.getAttribute('data-bkbg-infinite') !== '0';
		var autoScrollEnabled = settingsRoot.getAttribute('data-bkbg-autoscroll') === '1';
		var autoScrollSpeed = parseFloat(settingsRoot.getAttribute('data-bkbg-autoscroll-speed') || '40');
		if (!isFinite(autoScrollSpeed) || autoScrollSpeed < 0) autoScrollSpeed = 40;
		var autoScrollPauseOnHover = settingsRoot.getAttribute('data-bkbg-autoscroll-hover') !== '0';

		var prev = root.querySelector('[data-bkbg-nav="prev"], .bkbg-tm-prev');
		var next = root.querySelector('[data-bkbg-nav="next"], .bkbg-tm-next');
		var dots = root.querySelectorAll('[data-bkbg-dot]');
		var progressBar = root.querySelector('.bkbg-tm-progress-bar');
		...
		function getSlides() {
			return track.querySelectorAll('.bkbg-tm-slide');
		}
	...
	function initAll() {
		var blocks = document.querySelectorAll('.wp-block-blockenberg-team-members .bkbg-tm-carousel');
		for (var i = 0; i < blocks.length; i++) initCarousel(blocks[i]);
	}
